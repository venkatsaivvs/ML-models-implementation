...
2. compute_mae
3. _compute_impurity
4. _split_by_threshold
5. _variance_reduction
6. _find_best_split
7. _create_leaf
8. _build_tree
9. fit
...
class DecisionTreeRegressor:
    ...
    def _split_by_threshold(self, values: np.ndarray, y: np.ndarray, threshold: float):
        """Split target values into left/right based on values <= threshold"""
        mask_left = values <= threshold
        left_y = y[mask_left]
        right_y = y[~mask_left]
        return left_y, right_y
    
    def _variance_reduction(self, parent_y: np.ndarray, left_y: np.ndarray, right_y: np.ndarray) -> float:
        """Calculate variance reduction from splitting parent into left/right"""
        n = parent_y.size
        if n == 0:
            return 0.0
        ...
        variance_reduction = parent_impurity - (left_weight * left_impurity + right_weight * right_impurity)
    ...
    def _find_best_split(self, X: np.ndarray, y: np.ndarray):
        ...
        for feature_idx in range(num_features):
            ...
            for threshold in candidate_thresholds:
                ...
                if (left_y.size < self.min_samples_leaf or 
                    right_y.size < self.min_samples_leaf):
                    continue
                
                variance_reduction = self._variance_reduction(y, left_y, right_y)
                ...
                if variance_reduction > best_split["variance_reduction"]:
                    ...
                    best_split["variance_reduction"] = float(variance_reduction)
                    best_split["left_indices"] = np.where(left_mask)[0]
                    best_split["right_indices"] = np.where(right_mask)[0]
        
        return best_split
    
    def _create_leaf(self, y: np.ndarray):
        """Create a leaf node with the mean/median of target values"""
        if len(y) == 0:
    ...
    def predict(self, X):
        ...
        for x in X:
            prediction = self._predict_single(x, self.tree_)
    ...
    def print_tree(self, node=None, depth=0, feature_names=None):
        ...
        if node["type"] == "leaf":
            target_stats = node.get("target_stats", {})
            print(f"{indent}Leaf: value={node['value']:.3f}, samples={node['samples']}, stats={target_stats}")
        else:
            feature_name = f"feature_{node['feature_index']}"
            if feature_names is not None and node['feature_index'] < len(feature_names):
                feature_name = feature_names[node['feature_index']]
            
            print(f"{indent}{feature_name} <= {node['threshold']:.2f} [VR={node['variance_reduction']:.3f}, samples={node['samples']}]")
